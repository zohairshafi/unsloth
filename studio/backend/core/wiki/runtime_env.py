...
WIKI_ENV_SPECS: tuple[WikiEnvSpec, ...] = (
    ...
    WikiEnvSpec(
        name = "UNSLOTH_WIKI_LLM_PREFER_UPSTREAM",
        kind = "bool",
        default = "true",
        description = "Prefer upstream model calls for wiki maintenance/extraction flows before local fallback.",
    ),
    WikiEnvSpec(
        name = "UNSLOTH_WIKI_LLM_THINKING_ENABLED",
        kind = "bool",
        default = "true",
        description = "Enable reasoning/thinking hints for wiki LLM calls.",
    ),
    WikiEnvSpec(
        name = "UNSLOTH_WIKI_LLM_REASONING_STYLE",
        kind = "string",
        default = "reasoning_effort",
        description = "Reasoning control style for wiki LLM calls ('reasoning_effort' or 'enable_thinking').",
    ),
    WikiEnvSpec(
        name = "UNSLOTH_WIKI_LLM_REASONING_EFFORT",
        kind = "string",
        default = "high",
        description = "Reasoning effort level for wiki LLM calls when style is reasoning_effort ('low'|'medium'|'high').",
    ),
    WikiEnvSpec(
        name = "UNSLOTH_WIKI_LLM_PRESERVE_THINKING",
        kind = "bool",
        default = "false",
        description = "Preserve prior <think> traces for wiki LLM calls when template supports it.",
    ),
    WikiEnvSpec(
        name = "UNSLOTH_LLM_UPSTREAM_BASE_URL",
        kind = "string",
        default = "",
        description = "Upstream OpenAI-compatible API endpoint (base URL).",
    ),
    WikiEnvSpec(
        name = "UNSLOTH_LLM_UPSTREAM_API_KEY",
        kind = "string",
        default = "",
        description = "API key used for upstream OpenAI-compatible requests.",
    ),
    WikiEnvSpec(
        name = "UNSLOTH_LLM_UPSTREAM_MODEL",
        kind = "string",
        default = "",
        description = "Default upstream model name used when requests ask for default/current.",
    ),
    WikiEnvSpec(
    ...
    WikiEnvSpec(
        name = "UNSLOTH_WIKI_ENGINE_RANKING_TYPE_MIX_ENABLED",
        kind = "bool",
        default = "true",
        description = "Reserve top ranking slots across analysis/entity/concept page types.",
    ),
    WikiEnvSpec(
        name = "UNSLOTH_WIKI_ENGINE_RANKING_TYPE_MIX_WINDOW_PAGES",
        kind = "int",
        default = "24",
        description = "Top-N ranking window where type mix quotas are applied.",
        minimum = 1,
    ),
    WikiEnvSpec(
        name = "UNSLOTH_WIKI_ENGINE_RANKING_TYPE_MIX_ANALYSIS_RATIO",
        kind = "float",
        default = "0.60",
        description = "Target share of analysis pages in the type-mix ranking window.",
        minimum = 0.0,
        maximum = 1.0,
    ),
    WikiEnvSpec(
        name = "UNSLOTH_WIKI_ENGINE_RANKING_TYPE_MIX_ENTITY_RATIO",
        kind = "float",
        default = "0.20",
        description = "Target share of entity pages in the type-mix ranking window.",
        minimum = 0.0,
        maximum = 1.0,
    ),
    WikiEnvSpec(
        name = "UNSLOTH_WIKI_ENGINE_RANKING_TYPE_MIX_CONCEPT_RATIO",
        kind = "float",
        default = "0.20",
        description = "Target share of concept pages in the type-mix ranking window.",
        minimum = 0.0,
        maximum = 1.0,
    ),
    WikiEnvSpec(
